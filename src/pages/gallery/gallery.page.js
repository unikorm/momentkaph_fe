...
export default class GalleryPage {
  #reviews = new Carousel(GALLERY_REVIEWS.length - 1);
  #container = null;
  ...
  #setupHoverTiles() {
    this.#container.querySelectorAll('.image_container').forEach((el) => {
  ...
  #setupLanguageSwitcher() {
    const buttons = this.#container.querySelectorAll('.language_switcher button');
    const paint = () => {
      buttons.forEach((btn) => btn.classList.toggle('activeLanguage', btn.dataset.lang === i18n.currentLocale));
    };
    buttons.forEach((btn) => btn.addEventListener('click', () => {
      void i18n.setLocale(btn.dataset.lang).then(paint);
    }));
    paint();
  }
}
